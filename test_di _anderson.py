...
beta = abs(np.quantile(signal, alpha/2))
...
def Anderson(rumore,alpha,tau_max) :
    ...
    def Rho(segnale,tau_max) :
        ...
        gamma_zero = np.dot(segnale[1:tau_max], segnale_t[1:tau_max])
        for t in range(0,tau_max):
            gamma[t+1] = np.dot(segnale[1:tau_max-t], segnale_t[1+t:tau_max])
...
fig = plt.figure('Stima della covarianza campionaria normalizzata')
#for i in range (tau_max+1) :
    #plt.plot(i, rho, linewidth=1.5)
#plt.plot(range(0,tau_max+1),rho)
